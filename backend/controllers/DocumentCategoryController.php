...
use backend\models\documentcategory\AddDocumentCategories;
use backend\models\documentcategory\searchDocumentcategory;
use backend\models\documentcategory\UpdateDocumentCategories;
...
class DocumentCategoryController extends Controller {
    ...
    public function actionIndex() {
		$searchModel  = Yii::createObject(searchDocumentcategory::className());
    ...
    */
    ...
    public function actionCreate() {
        $data = array();
        $model = new AddDocumentCategories();
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
			$postval 			= Yii::$app->request->post('AddDocumentCategories');
            $model->status   	= '1';
            if ($model->insertdata()) {
                Yii::$app->session->setFlash('item', 'The new category has been created successfully!');
    ...
    */
    public function actionUpdate($id) {
        ...
        $updateNews	 		= new UpdateDocumentCategories();
        $model 		 		= $updateNews->findIdentity($id);
        $userpost 	 		= Yii::$app->request->post('UpdateDocumentCategories');

        if(isset($userpost) && !empty($userpost)) {
				if($model->load(Yii::$app->request->post()) && $model->updatedata($id)) {
					Yii::$app->session->setFlash('item', 'The category has been updated successfully!');
					return $this->redirect(['index']);				
				} else {
					Yii::$app->session->setFlash('item', 'Please enter valid values for all the fields!');
				}
