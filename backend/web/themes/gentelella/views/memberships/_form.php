...
<div class="x_content">
    ...
    <?php
		...
		//echo $form->field($modelImageUpload, 'image')->fileInput()->label('Image upload');   
		...
		echo $form->field($model, 'amount', ['inputOptions' => [
				'placeholder' => 'Price',
				'class' => "form-control col-md-7 col-xs-12",
		]])->textInput(['autofocus' => true])->label('Price <span class="required">*</span>');

		$plan = array("MONTH"=>"Month","YEAR"=>"Year");
		echo $form->field($model, 'membership_type')->dropDownList($plan)->label('Membership Type <span class="required">*</span>');
		
		$packages = array("USER"=>"User Subscription","ADS"=>"Advertisement Subscription");
		echo $form->field($model, 'package_type')->dropDownList($packages);
