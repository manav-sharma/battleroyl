...
class BannerController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
			$postedVal = Yii::$app->request->post('AddBannerForm');
			$checkStatus = $model->checkBanner($postedVal); 
			if(empty($checkStatus)) {
				...
				Yii::$app->session->setFlash('item', 'Banner is alreday assigned for the selected page!');
    ...
    public function actionUpdate($id) {
        ...
        if(isset($userpost) && !empty($userpost)) {
            #######################= FILE UPLOAD =######################
			$checkStatus = $updateBanner->checkBanner($userpost,$id); 
			if(empty($checkStatus)) {
				$model->bannerImage = '';
				if (Yii::$app->request->isPost) {
					$modelBannerPictureUpload->bannerImage = UploadedFile::getInstance($modelBannerPictureUpload, 'bannerImage');
